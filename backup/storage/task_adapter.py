...
def get_by_filter(filter_instance):
    ...
    """
    result = []
    # in this query we get all available tasks (our personal tasks and all tasks
    # pur in projects)
    query = _get_available_tasks().select().\
        join(User, on=((Task.creator == User.id) |
                       (Task.receiver == User.id))).\
