...
#### command line
\# todo
--adduser (name) (password) \[-nickname\] (name) \[-project\] (name)
Добавляет пользователя в базу данных с указанным именем и паролем. Опционально можно задать никнейм, по умолчанию он будет равен login. Также эта команда применима к добавлению пользователя в проект, текущий пользователь сессии необходим обладать правами на это.
--addtask (name) (date) \[-period\] (crontab style) \[-project\] (name) \[-priority\] (priority)
Добавляет таску для текущего пользователя, или же создает ее проекте. Все происходит с указанным приоритетом, по умолчанию medium (без определенных прав в проекте можно создать только low priority таску). Можно указать также период повторения таски после указанного времени, в формате * * * * *, минуты, часы, дни, месяца, годы.
--createproject (name)
Создать проект с правами администратора у текущего пользователя
#### search bar
\# todo
...
* **26.05.2018** - (current_date) --- _ведется создание веб интерфейса_

# Fast todo:
( Список меняется )
- изменить в библиотеке все на енамы а не дикты и переделать с консоли со строк на цифры
- доделать добавление чайлд таска, просмотра и его изменения 
- пофиксить время тасок (кривое задание + отображение)
- добавить метку что таска проектная (и соответственно имя проекта (вариант реализации: просто присобачить к доменному обьекту доп поле, которое по сути будет являться тайтлом проекта по айди, звучит не плохо))
- добавить страницу отображения именно тасок специфичного проекта (вместе с этим отобразить доп карту самого проекта с описанием и т д или же сайд вью вместе со списком юзеров в проекте)
- просмотр пользователей, кастомизация соответственно
- добавление в проекты пользователей и нормальное отображение пользователей при добавлении к проекту / таске проекта
- главная страница которая несет хоть какую-то информацию
- фикс карточек проектов
