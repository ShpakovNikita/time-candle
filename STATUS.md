...
Планируется создать класс главный пользователь, который будет являться базовым классом для всего локального приложения. Данный класс является потомком класса пользователь. У Главного Пользователя имеется список личных задач (список классов задача, поле принадлежности которого является текущий пользователь). Для групповых задач будет использоваться класс группа, в которой будет список доступных пользователей. Главный пользователь по умолчанию входит в создаваемую группу (у него существует поле список принадлежности к группам). Групповые задания являются также простым заданием, однако поле принадлежности к группе которой не является нулевое. У него целью может являться целью другой пользователь из группы. Каждое задание может иметь в себе список под заданий, создавая тем самым иерархию. Подзадание имеет не обязательно той же цели (пользователя), что и задание предок (обусловлено разделением труда между людьми, у кого-то лучше одно получается и ему нужна его помощь с заданием. Завершение задания рекурсивно завершает все подзадания. В случае работы с вебом, пользователь не единственный. Лидерство над группой имеет смысл быть передаваемым. Поэтому у группы появляется поле хранения лидера. Пользователь может покинуть группу. Группа в данном случае по сути и есть проект. Только пользователь равный лидеру может назначать задания для других пользователей с любым приоритетом и повышать его для любых заданий. Добавление пользователя в работу над проектом будет проходить из существующих пользователей по приглашению, запрос которого надо будет подтвердить для добавления себя как часть проекта/группы. В проекте информация о пользователях берётся из базы данных (возможно будет использована технология MySQL, в крайнем случае при нехватке времени, если это будет возможно, обойдемся простыми JSON запросами). Таким образом, задача сводится к созданию некоторой связанной ооп модели взаимодействия объектов между собой. Данная модель намерена дать лишь поверхностное представление об архитектуре, так как подробнее уточнить детали мне прийдется ещё в процессе создания, ведь, к сожалению, я не имею опыта в создании подобных локальных и веб приложений для полного уточнения своей дороги.
